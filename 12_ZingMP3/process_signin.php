...
if($stmt){
    ...
    if(mysqli_stmt_execute($stmt)) {
        ...
        if(mysqli_stmt_num_rows($stmt) == 1) {
            mysqli_stmt_bind_result($stmt, $user_id, $user_name, $user_img, $user_email, $user_password, $token, $status, $token_verification);
            if(mysqli_stmt_fetch($stmt)) {
                if($status == 1) {
                    ...
                    $_SESSION['error'] = 'Kiểm tra hộp thư của bạn để xác thực tài khoản!';
                }
            }
        } else {
